...
{
    for (float i = -1.1; i <= 1.1; i += 0.025)
    {
        if (a_condition == 1)
        {
            print_heart(i, a_y, fast_check(i, -0.33, 0.33, '|'));
        }
        else
        {
...
void Heart::for_message(float a_y, int a_condition)
{
    ...
    for (float i = -1.1; i <= 1.1; i += 0.025)
    {
        if (a_condition == 1)
        {
            print_message(i, a_y, fast_check(i, -0.06, 0.06, '|'), 0, k1);
        }
        elif (a_condition == 2)
        {
            print_message(i, a_y, fast_check(i, -0.12, 0.12, '|'), 1, k2);
        }
        else
        {
            print_message(i, a_y, fast_check(i, -0.08, 0.08, '|'), 2, k3);
        }
    }
}
...
{
    for (float y = 1.3; y >= -0.7; y -= 0.06)
    {
        if (fast_check(y, 0.5, 0.55, '&'))
        {
            for_message(y, 1);
        }
        elif (fast_check(y, 0.4, 0.45, '&'))
        {
            for_message(y, 2);
        }
        elif (fast_check(y, 0.25, 0.3, '&'))
        {
            for_message(y, 3);
        }
        elif (fast_check(y, 0.2, 0.6, '&'))
        {
            for_heart(y, 1);
        }
        else
